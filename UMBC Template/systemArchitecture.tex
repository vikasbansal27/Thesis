...
\For{$i \gets 1 \textrm{ to } FirstOuput.length$}
	\For{$j \gets 1\textrm{ to } DTEP$}
		\If{$distance(FirstOuput[i],DTEP[j]) < 5$}
			\State $DTEPFiltered[i].append(DTEP[j])$
		\EndIf
	\EndFor
\EndFor
\State $FinalOutput = SecondPass(ColorsOrig,FirstOuput,DTEPFiltered)$
...
\While {$\textbf{true}$}
	\For{$i \gets 1 \textrm{ to } ColorsRep.length$}
		\For{$j \gets 1\textrm{ to } DTEPSampled$}
	        ...
	        \If{$NewCost > cost$} 
	        		\State $ColorsRep[i] = Value$ \Comment{increased cost, Rejecting the color}
	        	\EndIf
	        \EndFor
	      \EndFor
